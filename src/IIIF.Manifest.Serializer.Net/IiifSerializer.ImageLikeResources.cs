...
public static partial class IiifSerializer
{
    private static JObject WriteV3Thumbnail(Thumbnail thumbnail)
    {
        ...
        if (thumbnail.Format is not null) obj["format"] = thumbnail.Format;

        if (thumbnail.Height is not null) obj["height"] = thumbnail.Height.Value;

        if (thumbnail.Width is not null) obj["width"] = thumbnail.Width.Value;
    ...
    private static Thumbnail ReadV3Thumbnail(JObject obj)
    {
        ...
        if ((string?)obj["format"] is { } format) thumbnail.SetFormat(format);

        if ((int?)obj["height"] is { } height) thumbnail.SetHeight(height);

        if ((int?)obj["width"] is { } width) thumbnail.SetWidth(width);
    ...
    private static JObject WriteV3Logo(Logo logo)
    {
        ...
        if (logo.Format is not null) obj["format"] = logo.Format;

        if (logo.Height is not null) obj["height"] = logo.Height.Value;

        if (logo.Width is not null) obj["width"] = logo.Width.Value;
    ...
    private static Logo ReadV3Logo(JObject obj)
    {
        ...
        if ((string?)obj["format"] is { } format) logo.SetFormat(format);

        if ((int?)obj["height"] is { } height) logo.SetHeight(height);

        if ((int?)obj["width"] is { } width) logo.SetWidth(width);
