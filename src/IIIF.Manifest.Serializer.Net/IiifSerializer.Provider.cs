...
public static partial class IiifSerializer
{
    private static void WriteV3Provider<TBaseNode>(BaseNode<TBaseNode> node, JObject obj) where TBaseNode : BaseNode<TBaseNode>
    {
        ...
        if (providers.Count > 0) obj["provider"] = new JArray(providers);
    }

    private static void ReadV3Provider<TBaseNode>(JObject obj, BaseNode<TBaseNode> node) where TBaseNode : BaseNode<TBaseNode>
    {
        foreach (var providerObj in obj["provider"]?.OfType<JObject>() ?? Enumerable.Empty<JObject>()) node.AddProvider(ReadV3ProviderEntry(providerObj));
    }
    ...
    private static JObject WriteV3ProviderEntry(Provider provider)
    {
        ...
        if (homepages.Count > 0) obj["homepage"] = new JArray(homepages);

        var logos = provider.Logo.Select(WriteV3Logo).ToList();
        if (logos.Count > 0) obj["logo"] = new JArray(logos);

        var seeAlsos = provider.SeeAlso.Select(WriteV3SeeAlso).ToList();
        if (seeAlsos.Count > 0) obj["seeAlso"] = new JArray(seeAlsos);
    ...
    private static Provider ReadV3ProviderEntry(JObject obj)
    {
        ...
        if (labels.Count > 0) provider.SetLabel(labels);

        foreach (var homepageObj in obj["homepage"]?.OfType<JObject>() ?? Enumerable.Empty<JObject>()) provider.AddHomepage(ReadV3Homepage(homepageObj));

        foreach (var logoObj in obj["logo"]?.OfType<JObject>() ?? Enumerable.Empty<JObject>()) provider.AddLogo(ReadV3Logo(logoObj));

        foreach (var seeAlsoObj in obj["seeAlso"]?.OfType<JObject>() ?? Enumerable.Empty<JObject>()) provider.AddSeeAlso(ReadV3SeeAlso(seeAlsoObj));
