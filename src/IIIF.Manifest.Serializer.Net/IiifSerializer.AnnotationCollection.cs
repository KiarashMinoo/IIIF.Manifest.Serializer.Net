...
public static partial class IiifSerializer
{
    private static JObject WriteV3AnnotationCollection(AnnotationCollection annotationCollection)
    {
        ...
        if (annotationCollection.Total is not null) obj["total"] = annotationCollection.Total.Value;

        if (annotationCollection.First is not null) obj["first"] = annotationCollection.First;

        if (annotationCollection.Last is not null) obj["last"] = annotationCollection.Last;
    ...
    private static AnnotationCollection ReadV3AnnotationCollection(JObject obj)
    {
        ...
        if ((int?)obj["total"] is { } total) annotationCollection.SetTotal(total);

        if ((string?)obj["first"] is { } first) annotationCollection.SetFirst(first);

        if ((string?)obj["last"] is { } last) annotationCollection.SetLast(last);
