...
public static partial class IiifSerializer
{
    private static JObject WriteV3Rendering(Rendering rendering)
    {
        ...
        if (rendering.Format is not null) obj["format"] = rendering.Format;
    ...
    private static Rendering ReadV3Rendering(JObject obj)
    {
        ...
        if ((string?)obj["format"] is { } format) rendering.SetFormat(format);
    ...
    private static JObject WriteV3Homepage(Homepage homepage)
    {
        ...
        if (homepage.Label is not null) WriteLanguageMap(obj, "label", [homepage.Label]);

        if (homepage.Format is not null) obj["format"] = homepage.Format;
    ...
    private static Homepage ReadV3Homepage(JObject obj)
    {
        ...
        if ((string?)obj["format"] is { } format) homepage.SetFormat(format);
    ...
    private static JObject WriteV3SeeAlso(SeeAlso seeAlso)
    {
        ...
        if (seeAlso.Label is not null) WriteLanguageMap(obj, "label", [seeAlso.Label]);

        if (seeAlso.Format is not null) obj["format"] = seeAlso.Format;

        if (seeAlso.Profile is not null) obj["profile"] = seeAlso.Profile;
    ...
    private static SeeAlso ReadV3SeeAlso(JObject obj)
    {
        ...
        if ((string?)obj["type"] is { } type) seeAlso.SetType(type);

        if ((string?)obj["format"] is { } format) seeAlso.SetFormat(format);

        if ((string?)obj["profile"] is { } profile) seeAlso.SetProfile(profile);

        if (ReadLabels(obj["label"]).FirstOrDefault()?.Value is { } label) seeAlso.SetLabel(label);
