...
public static class IiifPresentationVersionDetector
{
    ...
    {
        if (string.IsNullOrWhiteSpace(json)) return IiifPresentationVersion.Unknown;
    ...
    {
        if (token is not JObject obj) return IiifPresentationVersion.Unknown;

        var contextVersion = DetectFromContext(obj["@context"]);
        if (contextVersion != IiifPresentationVersion.Unknown) return contextVersion;

        var type = (string?)obj["type"] ?? (string?)obj["@type"];
        if (!string.IsNullOrWhiteSpace(type))
        {
            if (type is "Manifest" or "Collection" or "Canvas" or "AnnotationPage" or "Annotation") return IiifPresentationVersion.V3_0;

            if (type.StartsWith("sc:") || type.StartsWith("oa:") || type.StartsWith("dctypes:")) return IiifPresentationVersion.V2_1;
        }

        if (obj["sequences"] is not null || obj["@id"] is not null || obj["@type"] is not null) return IiifPresentationVersion.V2_1;

        if (obj["items"] is not null || obj["id"] is not null || obj["type"] is not null) return IiifPresentationVersion.V3_0;
    ...
    {
        if (context is null) return IiifPresentationVersion.Unknown;
        ...
        if (context.Type == JTokenType.Array)
        {
            foreach (var item in context.Children())
            {
                ...
                if (version != IiifPresentationVersion.Unknown) return version;
            }
        ...
        if (string.IsNullOrWhiteSpace(value)) return IiifPresentationVersion.Unknown;

        if (value.Contains("/presentation/4/")) return IiifPresentationVersion.V4_0_Rc;

        if (value.Contains("/presentation/3/")) return IiifPresentationVersion.V3_0;
        ...
        if (value.Contains("/presentation/2.0/")) return IiifPresentationVersion.V2_0;
        ...
        if (value.Contains("/presentation/2/")) return IiifPresentationVersion.V2_1;
        ...
        // context URL.
        if (value.Contains("shared-canvas.org")) return IiifPresentationVersion.Metadata_1_0;
