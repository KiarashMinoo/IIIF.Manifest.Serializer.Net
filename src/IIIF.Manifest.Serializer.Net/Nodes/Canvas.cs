...
namespace IIIF.Manifests.Serializer.Nodes
{
    ...
    public class Canvas : BaseNode<Canvas>, IDimensionSupport<Canvas>
    {
        ...
        private IEnumerable<Annotation> GetPaintingAnnotations() => PrimaryAnnotationPage?.Items.OfType<Annotation>() ?? [];
        ...
        private static Annotation ToAnnotation(Audio audio) => new(audio.Id, audio.Resource, audio.On);
        private static Annotation ToAnnotation(Video video) => new(video.Id, video.Resource, video.On);
        private static AnnotationPage ToAnnotationPageReference(OtherContent otherContent) => new(otherContent.Id);
    }
}
