...
public class Collection : BaseNode<Collection>, IViewingDirectionSupport<Collection>
{
    ...
    public Collection(string id, Label label) : this(id)
    {
        AddLabel(label);
    }

    public Collection(string id, IReadOnlyCollection<Label> labels) : this(id)
    {
        SetLabel(labels);
    }
    ...
    public Collection RemoveManifest(string manifestId)
    {
        ...
        if (existing is not null) RemoveItem(existing);
    ...
    {
        foreach (var existing in Items.OfType<TItem>().ToList()) RemoveItem(existing);

        foreach (var item in replacements) AddItem(item);
    }

    private void ReplaceMembers(IEnumerable<object> members)
    {
        foreach (var existing in Items.ToList()) RemoveItem(existing);
