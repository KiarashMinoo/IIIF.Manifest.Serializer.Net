using System.Collections.Generic;
using IIIF.Manifests.Serializer.Attributes;
using IIIF.Manifests.Serializer.Helpers;
using IIIF.Manifests.Serializer.Shared;
using IIIF.Manifests.Serializer.Shared.Service;
using Newtonsoft.Json;

namespace IIIF.Manifests.Serializer.Properties.Services;

/// <summary>
/// IIIF Authentication API 1.0 Service - provides authentication flow information.
/// Used for login, clickthrough, kiosk, and external authentication patterns.
/// </summary>
[AuthAPI("0", Notes = "Auth API 1.0 service for content access control.")]
public class AuthService : BaseItem<AuthService>, IBaseService
{
    public const string LabelJName = "label";
    public const string HeaderJName = "header";
    public const string DescriptionJName = "description";
    public const string ConfirmLabelJName = "confirmLabel";
    public const string FailureHeaderJName = "failureHeader";
    public const string FailureDescriptionJName = "failureDescription";

    [AuthAPI("0")]
    [JsonProperty(IBaseService.ProfileJName)]
    public string Profile
    {
        get => GetElementValue(x => x.Profile)!;
        private set => SetElementValue(value);
    }

    [AuthAPI("0")]
    [JsonProperty(LabelJName)]
    public string? Label
    {
        get => GetElementValue(x => x.Label);
        private set => SetElementValue(value);
    }

    [AuthAPI("0")]
    [JsonProperty(ConfirmLabelJName)]
    public string? ConfirmLabel
    {
        get => GetElementValue(x => x.ConfirmLabel);
        private set => SetElementValue(value);
    }

    [AuthAPI("0")]
    [JsonProperty(HeaderJName)]
    public string? Header
    {
        get => GetElementValue(x => x.Header);
        private set => SetElementValue(value);
    }

    [AuthAPI("0")]
    [JsonProperty(DescriptionJName)]
    public string? Description
    {
        get => GetElementValue(x => x.Description);
        private set => SetElementValue(value);
    }

    [AuthAPI("0")]
    [JsonProperty(FailureHeaderJName)]
    public string? FailureHeader
    {
        get => GetElementValue(x => x.FailureHeader);
        private set => SetElementValue(value);
    }

    [AuthAPI("0")]
    [JsonProperty(FailureDescriptionJName)]
    public string? FailureDescription
    {
        get => GetElementValue(x => x.FailureDescription);
        private set => SetElementValue(value);
    }

    /// <summary>
    /// Creates a new Auth API 1.0 service.
    /// </summary>
    /// <param name="id">Service identifier (login/token/logout endpoint URL)</param>
    /// <param name="profile">Auth profile (login/clickthrough/kiosk/external/token/logout)</param>
    public AuthService(string id, string profile)
        : base(id, string.Empty, "http://iiif.io/api/auth/0/context.json")
    {
        Profile = profile;
    }

    /// <summary>
    /// Sets the user-facing label for the authentication interaction.
    /// </summary>
    public AuthService SetLabel(string label)
    {
        Label = label;
        return this;
    }

    /// <summary>
    /// Sets the header text for the authentication interaction window.
    /// </summary>
    public AuthService SetHeader(string header)
    {
        Header = header;
        return this;
    }

    /// <summary>
    /// Sets the description text explaining the authentication requirement.
    /// </summary>
    public AuthService SetDescription(string description)
    {
        Description = description;
        return this;
    }

    /// <summary>
    /// Sets the label for the confirm button in clickthrough interactions.
    /// </summary>
    public AuthService SetConfirmLabel(string confirmLabel)
    {
        ConfirmLabel = confirmLabel;
        return this;
    }

    /// <summary>
    /// Sets the header text shown when authentication fails.
    /// </summary>
    public AuthService SetFailureHeader(string failureHeader)
    {
        FailureHeader = failureHeader;
        return this;
    }

    /// <summary>
    /// Sets the description text shown when authentication fails.
    /// </summary>
    public AuthService SetFailureDescription(string failureDescription)
    {
        FailureDescription = failureDescription;
        return this;
    }
}