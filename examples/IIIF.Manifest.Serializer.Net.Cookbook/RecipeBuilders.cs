...
internal static class RecipeBuilders
{
    ...
    internal static string Id(string recipe, string path) => $"{Base}/{recipe}/{path}";

    internal static string CanvasId(string recipe, string page) => Id(recipe, $"canvas/{page}");

    internal static Manifest NewManifest(string recipe, string label, string file = "manifest.json") => new(Id(recipe, file), new Label(label));

    internal static Manifest NewManifest(string recipe, Label label, string file = "manifest.json") => new(Id(recipe, file), label);

    internal static Manifest NewManifest(string recipe, Label primary, Label secondary) => new Manifest(Id(recipe, "manifest.json"), primary).AddLabel(secondary);
